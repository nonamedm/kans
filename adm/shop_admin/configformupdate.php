...
// ================= 커스터 마이징 - 페이조아 관련 추가 [한국원자력아카데미] =================
// DB 컬럼 추가
$check_query = "
SELECT 1 FROM Information_schema.columns
WHERE table_schema = '".G5_MYSQL_DB."' 
AND table_name = '".$g5['g5_shop_default_table']."' 
AND column_name = 'de_payjoa_mid'";
$check_result = sql_query($check_query);
$check_info = sql_fetch_array($check_result);

// 컬럼 체크 후 없을 경우
if(!$check_info2['1']) {
	$alter_query = "ALTER TABLE ".$g5['g5_shop_default_table']." ADD COLUMN de_payjoa_mid VARCHAR(255) NOT NULL";
...
// ================= 커스터 마이징 - 페이조아 관련 추가 [한국원자력아카데미] =================
$sql .= ", de_payjoa_mid = '".$_POST['de_payjoa_mid']."' ";
