...
<div style="margin:30px auto;width:600px;border:10px solid #f7f7f7">
    <div style="border:1px solid #dedede">
        ...
        <div style="margin:20px 0 0;padding:30px 30px 50px;min-height:200px;height:auto !important;height:200px;border-bottom:1px solid #eee">
            <p>이메일 : <?php echo $wr_email ?></p>
            <p>문의 내용 : <?php echo $wr_content ?></p>
        </div>
		<?php for ($i=0; $i<count($upload); $i++) { 
		if ($upload[$i]['file']) {
		?> 
		<a download href="<?php echo G5_DATA_URL.'/file/'.$bo_table.'/'.$upload[$i]['file']; ?>"> 첨부파일 다운로드</a>
		<?php }} ?>
