	...
	$load_cal="<tr height=100>";
		...
		for($i = $row = 1; $i < $VAR['count']; $i++, $row++)
		{
			...
			$idx="";
				...
				$sql = "SELECT * FROM ".$write_table." WHERE wr_1 = '".$dateymd."' OR wr_2 = '".$dateymd."' OR ('".$dateymd."' BETWEEN wr_1 AND wr_2) ORDER BY wr_1, wr_id DESC ";
				$query=sql_query($sql);
...
//기타추가
					...
					$load_subject .= "<li><h5>교육시간</h5><p>".$row2['wr_1']." ~ ".$row2['wr_2']."</p></li>";
					$load_subject .= "<li><h5>교육장소</h5><p>".$row2['wr_content']."</p></li>";
					$load_subject .= "<li><h5>신청인원</h5><p>".$row2['wr_3']."</p></li>";
// 스토리보드 금액추가
					$load_subject .= "<li><h5>금액</h5><p><b>30,000</b></p></li>";
					$load_subject .= "<ul>";
					$load_subject .= "<input type='button' value='신청하기'; onclick=document.location.href='http://www.kans.re.kr/cal/cal_write1.php#n'> ";
