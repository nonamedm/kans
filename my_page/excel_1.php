...
// API 회원의 수강내역 가져오기
$url = "https://www.kanselearning.kr/mediopia/calleduCompleteStudentInfoListKanselearningJSON?USER_ID={$member['mb_id']}{$api_where}";
	...
	<?php if(count($data_)){
		for($i = 0; $i < count($data_); $i++) { ?>
			...
			<tr>
				...
				<td class='txt'></td><?php echo $pro_info['wr_content']; ?><!-- 교육장소 -->
				<td class='txt'><?php echo $data['wr_18']?></td><!-- 진행상태 -->
				<td class='txt'><?php echo ($data['wr_17'])?$data['wr_17']:"미결제";; ?></td><!-- 결제상태 -->
				<td class='txt'></td><!-- 금액 -->
				<td class='txt'><?php echo $data['wr_26']?></td><!-- 수료증 번호 -->
				<td class='txt'></td><!-- 비고 -->
				<td class='txt'></td><!-- 등록일 -->
			</tr>
		<?php $number--; }
