...
if($boardfile != ''){
	...
	if(strcmp($boardfile,'none'))	{
		...
		if(!($extension == 'hwp' || $extension == 'doc' || $extension == 'gif' || $extension == 'jpg' || $extension == 'ppt' || $extension == 'pdf')){
			echo"<script language=javascript>alert('[hwp,ppt,pdf,doc,gif,jpg,txt] 파일만 올릴수 있습니다');history.back();</script>";
			exit;
		}
...
$sql = "select max(fid)+1 as fid from board where bbs_id=4"; 
