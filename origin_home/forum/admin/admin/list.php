...
if ($board['bo_use_category']) {
    $is_category = true;
    $category_href = G5_MANAGE_URL.'/board.php?bo_table='.$bo_table;
...
if ($sop != 'and' && $sop != 'or')
	...
	if (!$sca && !$stx) {
		...
		if($from_notice_idx < 0)
			...
			for ($k=0; $k<$board_notice_count; $k++) {
				...
				$list[$i] = get_admin_list($row, $board, $board_skin_url, G5_IS_MOBILE ? $board['bo_mobile_subject_len'] : $board['bo_subject_len']);
	...
	if ($is_member && ($is_admin == 'super' || $group['gr_admin'] == $member['mb_id'] || $board['bo_admin'] == $member['mb_id']))
		...
		if(!$sst)
			...
			if($lineup1) {$sql_order= " order by wr_id asc "; }
			else if($linedown1) {$sql_order= " order by wr_id desc "; }
			if($lineup2) {$sql_order= " order by wr_9 asc ";}
			else if($linedown2) {$sql_order= " order by wr_9 desc ";}
			if($lineup3) {$sql_order= " order by wr_10 asc ";}
			else if($linedown3) {$sql_order= " order by wr_10 desc "; }
