...
$category_title = "교육신청관리";	//카테고리 제목
$sub_title = "교육신청관리";	//서브 타이틀
$sub_explan = ""; //페이지 설명
include_once G5_MANAGE_INCLUDE_PATH . "/admin_header.php";

$sql_common = " from schedule_submit";
$sql_search = " where (1) ";
...
<div class="status_area mb10">
		<div class="">
				<select name="sfl" id="sfl">
					<option value="wr_subject">교육명</option>
...
$("#sfl option[value='<?=$sfl?>']").prop("selected", "true");
	...
	<input type='hidden' name='write_table'  value="schedule_submit"/>
...
<table class="horizen">
	...
	<tbody>
		...
		<tr>
			...
			<td><?=$row[wr_2]?></td>
			<td>
				<select id="select_<?=$row[idx]?>" onchange="list_change('schedule_submit','wr_10',this.value,'<?=$row[idx]?>','<?=$row[wr_10]?>');">
					<option value="승인대기">승인대기</option>
					<option value="승인">승인</option>
					<option value="이수실패">이수실패</option>
    ...
    <div class="btn_area mt20">
		<div class="btn_area_left">
					...
					<a href="./excel.php?table=schedule_submit&sfl=<?=$sfl?>&stx=<?=$stx?>" class="btn_normal">엑셀저장</a>
		</div>
    </div>
<form>
