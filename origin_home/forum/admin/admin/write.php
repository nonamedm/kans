...
//게시판을 adm에서 만들시 자동으로 처리 YK 03.10 (단, 그룹이 소속되야지 나온다 !!)

$cate_t = sql_fetch("select * from g5_board where bo_table = '$bo_table'"); // 1차로현재 board의 정보를 가지고온다
$cate_gt = sql_fetch("select * from g5_group where gr_id = '$cate_t[gr_id]'"); // 2차로현재 board한 소속의 group명을 가지고온다 실제 이녀석이 title을 담당한다.

$g5['title'] = $cate_gt[gr_subject]." > ".$cate_t[bo_subject];	//title
$category_title = $cate_gt[gr_subject];	//카테고리 제목
$sub_title = $cate_t[bo_subject];	//서브 타이틀
$sub_explan = ""; //페이지 설명

$board_chk = $cate_gt[gr_id]; // board_chk에 자료가있으면 실행하도록 설정한다. 이 부분은 추 후 ADMIN_HEADER.PHP에서 역할을 담당한다.

//게시판을 adm에서 만들시 자동으로 처리 YK 03.10

//$category_title = "게시판관리";	//카테고리 제목
...
include_once("./admin_board_include.php");
...
$t_name = $board['bo_subject'];

$g5['title'] = $t_name.' '.$title_msg;
...
if ($w == '') {
	...
	if (!strstr($write['wr_option'], 'html')) {
		$wr_9 = "\n\n\n &gt; "
				 ."\n &gt; "
				 ."\n &gt; ".str_replace("\n", "\n> ", get_text($write['wr_9'], 0))
				 ."\n &gt; "
				 ."\n &gt; ";

	}
} else {
	$wr_9 = get_text($write['wr_9'], 0);
}
...
//간단소개용 에디터 생성작업 진행
$editor_html_simple = editor_html('wr_9', $wr_9, $is_dhtml_editor);
$editor_js_simple = '';
$editor_js_simple .= get_editor_js('wr_9', $is_dhtml_editor);
$editor_js_simple .= chk_editor_js('wr_9', $is_dhtml_editor);
...
$action_url = https_url(G5_MANAGE_DIR)."/write_update.php";

echo '<!-- skin : '.(G5_IS_MOBILE ? $board['bo_mobile_skin'] : $board['bo_skin']).' -->';
//echo ($board_skin_path.'/write.skin.php');
