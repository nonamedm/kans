...
<div id="bo_list" style="width:<?php echo $width; ?>">
	...
	<div class="state_area">
		...
		<div class="state_area_right">
			...
			<a href="#n" class="cate_btn btn_normal" onclick="layer_board('<?=$board[bo_subject]?> 연도수정','./board_cate.php?bo_table=<?=$bo_table?>');">연도수정</a>
		</div>
	</div>
	<!-- } 게시판 페이지 정보 및 버튼 끝 -->
				...
				<tbody>
					<?php for ($i=0; $i<count($list); $i++) { ?>
						<tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
							<td class="">
								<input type="hidden" name="s_wr_id[]" value="<?php echo $list[$i]['wr_id']; ?>">
								<input type="hidden" name="s_wr_10[]" value="<?php echo $list[$i]['wr_10']; ?>">
								<?php
							...
							<td class=""><a href="<?php echo $list[$i]['href'] ?>"><?php echo $list[$i]['subject'].".".$list[$i]['wr_1']; ?></a></td>
							<td class=""><a href="<?php echo $list[$i]['href'] ?>"><?php echo nl2br($list[$i]['wr_content']); ?></a></td>
							<td class=""><!-- <a href="#n" class="up">▲</a><a href="#" class="down">▼</a> --><input type="text" class="frm_input" size="7" value="<?php echo $list[$i]['wr_10']; ?>" onkeyup="chenge_order('<?php echo $bo_table; ?>', '<?php echo $list[$i]['wr_id']; ?>', 'wr_10', this.value);"></td>
		...
		<fieldset id="">
			...
			<form name="fsearch" method="get">
				...
				<select name="sfl" id="sfl">
					<option value="wr_subject"<?php echo get_selected($sfl, 'wr_subject', true); ?>>년</option>
					<option value="wr_content"<?php echo get_selected($sfl, 'wr_content'); ?>>내용</option>
					<option value="wr_subject||wr_content"<?php echo get_selected($sfl, 'wr_subject||wr_content'); ?>>년+내용</option>
				</select>
...
function resizeTopIframe(dynheight) { // 자식 iframe 리사이즈
	document.getElementById("layer_frame").height = parseInt(dynheight) + 10;
	$("#layer").css("height", parseInt(dynheight) + 10);
}
