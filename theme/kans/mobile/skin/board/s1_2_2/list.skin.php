		...
		<table>
			<tr>
				...
				<td>
					<div class="box">
						<ul>
							<li><a href="<?=$s1_2_2_url;?>"><input type="radio" name="s1_2" onclick="document.location.href='<?php echo $s1_2_2_url; ?>';" ><label for="">달력 형</label></a></li>
							<li><a href="<?=$s1_2_2_2_url;?>"><input type="radio" name="s1_2" checked="checked" onclick="document.location.href='<?php echo $s1_2_2_2_url; ?>';"><label for="">리스트 형</label></a></li>
						</ul>
					</div>
				</td>
			</tr>
				...
				<td>
					<?php
						...
						if(strstr($_SERVER['REMOTE_ADDR'], "211.170.81") || strstr($_SERVER['REMOTE_ADDR'], "192.168.45.13") || strstr($_SERVER['REMOTE_ADDR'], "58.124.26.184")){
							if($is_member){ echo "<a class='btn_request btn_ty2' data-val='".$list[$i]['wr_id']."'>신청하기</a> "; }
							else{ echo "<a href=\"./login.php?{$qstr}&amp;url=".urlencode($_SERVER['SCRIPT_NAME']."?bo_table=".$bo_table)."\" class='btn_ty2' data-val='".$list[$i]['wr_id']."'>신청하기</a> "; }
						}
						}
					?>
				</td>
