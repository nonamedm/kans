...
<!--<script>
    if(!g5_is_member) {
        alert("로그인후 이용 해 주세요.");
        goto_url ("http://kans.re.kr/admin/login.php?url=");
    }
</script>-->
...
<?php
	...
	// $write_href = false;
?>
<link href='<?php echo $board_skin_url ?>/fullcalendar/main.css' rel='stylesheet' />
<script src='<?php echo $board_skin_url ?>/fullcalendar/main.js'></script>

<?php
    $sql="SELECT wr_id, wr_subject, wr_1, wr_datetime FROM g5_write_forum_info";
...
<script>
      ...
      document.addEventListener('DOMContentLoaded', function() {
        ...
        var calendar = new FullCalendar.Calendar(calendarEl, {
          ...
          events: [
          <?php
             while($result= sql_fetch_array($datarow)){ ?>
            {
              title: "<?php echo $result['wr_subject']; ?>",
              start: "<?php echo $result['wr_1']; ?>",
			  url: "http://www.kans.re.kr/bbs/board.php?bo_table=forum_info&wr_id=<?php echo $result['wr_id']; ?>"
            },
          <?php } ?>
            ]
...
<!-- 게시판 목록 시작 -->
<div id="bo_list<?php if ($is_admin) echo "_admin"; ?>" class="respon_l" style="max-width: 1320px; margin: auto;">
	<?php if ($is_category) { ?>
		<nav id="bo_cate">
...
<div class="bd_search ct1">
	<fieldset id="bo_sch">
		...
		<form name="fsearch" method="get">
			...
			<input type="text" name="stx" value="<?php echo stripslashes($stx) ?>" placeholder="검색어를 입력해 주세요."  id="stx" class="input_ty" size="12" maxlength="20" onkeyup="color_ch(this.value);">
...
<script>
    ...
    function color_ch(a){
        var col_lenth = $('.col_subject').length;
        console.log(a);
        for(var k =1 ; k < col_lenth; k++ ){
            if($('.col_subject')[k].children[0].innerText.trim().includes(a)){
                if(stx == ''){
                    $('.col_subject > a').removeClass('color_ch');
                    $('.respon_l .div_td a').css('color','#000');
                }else{
                    $('.col_subject > a').addClass('color_ch');
                    $('.respon_l .div_td a').css('color','red');
                }
            }else{
                $('.col_subject > a').removeClass('color_ch');
                $('.respon_l .div_td a').css('color','#000');
    ...
    function stx_sh(){
        ...
        for(var k =1 ; k < col_lenth; k++ ){
            if($('.col_subject')[k].children[0].innerText.trim().includes(stx)){
                if(stx == ''){
                    $('.col_subject > a').removeClass('color_ch');
                    $('.respon_l .div_td a').css('color','#000');
                }else{
