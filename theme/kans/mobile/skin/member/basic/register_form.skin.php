...
<div class="mbskin">
	...
	<form name="fregisterform" id="fregisterform" action="<?php echo $register_action_url ?>" onsubmit="return fregisterform_submit(this);" method="post" enctype="multipart/form-data" autocomplete="off">
		...
		<div  id="register_form">
			...
			<ul class="option_check">
				<li><input type="radio" name="request_level" id="mb_level_1" value="2" required <?php echo $readonly ?>><label for="mb_level_1">교육생(개인) 가입</label></li>
				<li><input type="radio" name="request_level" id="mb_level_2" value="3" required <?php echo $readonly ?>><label for="mb_level_2">교육담당자(단체) 가입</label></li>
			</ul>
			<script type="text/javascript">
			$( document ).ready(function() {
				$( "input[name='request_level'][value='<?php echo $member['mb_level']; ?>']" ).prop( "checked", true);
			});
			</script>
			<?php } else { ?>
				<ul class="option_check"><li><?php echo ($member['mb_level'] == '2')?"개인회원":"기관회원";?></li></ul>
			<?php } ?>
			...
			<div class="tbl_frm01 tbl_wrap">
				<table>
                    ...
                    <tr> <!--기관회원가입용-->
                        <th scope="row"><label for="mb_2">소속<strong class="sound_only">필수</strong></label></th>
                        <td>
							<?php if($w == ""){ ?>
							<input type="text" name="mb_2" value="<?php echo $member['mb_2'] ?>" placeholder="소속" id="mb_2" class="frm_input1 open_pop <?php echo $required ?> <?php echo $readonly ?>" minlength="3" maxlength="20" <?php echo $required ?> <?php echo $readonly ?> onclick="$('#search_layer').find('input[id=stx]').val('');" readonly>
                            <a href="#n" class="btn_st1 btn_search open_pop" onclick="$('#search_layer').find('input[id=stx]').val('');">검색</a>
							<?php } else { ?>
							<input type="hidden" name="mb_2" value="<?php echo $member['mb_2'] ?>" readonly><?php echo $member['mb_2'] ?>
							<?php } ?>
                        </td>
                    </tr>
					<tr>
						...
						<td>
							...
							<?php } ?>
							<input type="text" id="reg_mb_name" placeholder="이름" name="mb_name" value="<?php echo get_text($member['mb_name']) ?>" <?php echo $required ?> <?php echo $readonly; ?> class="frm_input <?php echo $required ?> <?php echo $readonly ?>">
							<?php
					...
					<tr>
						...
						<td>
							<input type="text" name="mb_3" value="<?php echo $member['mb_3']; ?>" placeholder="710101" maxlength="6" style="border: 1px solid #e6e6e6; background: #f7f7f7;" class="input_bith">  - 
							<input type="text" name="mb_4" value="<?php echo $member['mb_4']; ?>" placeholder="1" size="2" maxlength="1" style="border: 1px solid #e6e6e6; background: #f7f7f7;" class="input_bith" >
					...
					<?php if($config['cf_use_hp']){ ?>
						<tr>
							<th scope="row"><label for="reg_mb_hp">핸드폰 번호<?php if ($config['cf_req_hp']) { ?><strong class="sound_only">필수</strong><?php } ?></label></th>
							<td><input type="text" name="mb_hp" required  placeholder="연락처(핸드폰)" value="<?php echo get_text($member['mb_hp']) ?>" id="reg_mb_hp" class="required frm_input <?php echo $config['cf_req_hp']?"required":""; ?> tel_input" maxlength="20" <?php echo $config['cf_req_hp']?"required":""; ?>></td>
						</tr>
					<?php } ?>
						...
						<th scope="row"><label for="reg_mb_5">방사선사 면허번호</label></th>
						<td>
							<input type="text" placeholder="방사선사 면허번호" name="mb_5" value="<?php echo isset($member['mb_5'])?$member['mb_5']:''; ?>" id="reg_mb_5" class="frm_input" />
						</td>
						</tr>
	...
	function fregisterform_submit(f)
	{
		...
		<?php if ($w == "" && ($config['cf_use_hp'] || $config['cf_cert_hp'])) {  ?>
		// 휴대폰번호 체크
