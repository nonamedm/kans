    ...
    <div class="flex-ev">
        ...
        for ($i=0; $row=sql_fetch_array($hresult); $i++)
        {
            ...
            if (file_exists($event_img)) { // 이벤트 이미지가 있다면 이미지 출력
                echo '<a href="'.$href.'" class="sev_img" style="background-image:url('.G5_DATA_URL.'/event/'.$row['ev_id'].'_m)"></a>'.PHP_EOL;
            } else { // 없다면 텍스트 출력
            ...
            echo '<a href="'.$href.'" class="sev_more">더보기</a>';
            // 이벤트 상품
            $sql2 = " select b.*
            ...
            for($k=1; $row2=sql_fetch_array($result2); $k++) {
                ...
                echo '<span>'.get_it_image($row2['it_id'], 230, 230, get_text($row2['it_name'])).'</span>'.PHP_EOL;
                echo '<a href="'.$item_href.'" class="ev_prd_tit">'.get_text(cut_str($row2['it_name'], 30)).'</a>'.PHP_EOL;
            ...
            if($k == 1) {
                echo '<ul class="no_prd">'.PHP_EOL;
                echo '<li class="sev_empty">등록된 상품이 없습니다.</li>'.PHP_EOL;
                echo '</ul>'.PHP_EOL;
            }
